...
import { Highlighter, Underline, MessageSquare, Pen, Trash2, MousePointer, ZoomIn, ZoomOut } from "lucide-react"
...
export function ToolBar({ file, onZoom, currentScale = 1.5 }: ToolBarProps) {
  ...
  const handleZoomIn = () => {
    if (onZoom) onZoom(currentScale + 0.25)
  }

  const handleZoomOut = () => {
    if (onZoom && currentScale > 0.5) onZoom(currentScale - 0.25)
  }
      ...
      <div className="flex flex-col gap-4 rounded-lg border bg-white p-4 shadow-sm">
        <Alert className="bg-blue-50 border-blue-200">
          ...
          <AlertDescription className="text-blue-700">
            {currentTool === "none"
              ? "Selection mode active. Click on annotations to select, drag to reposition, or delete them."
              : `${currentTool.charAt(0).toUpperCase() + currentTool.slice(1)} tool is active. Click on the PDF to add.`}
          </AlertDescription>
        </Alert>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className={currentTool === "none" ? "ring-2 ring-primary" : ""}
                >
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={handleZoomOut} disabled={!onZoom || currentScale <= 0.5}>
              <ZoomOut className="h-4 w-4 mr-1" />
              Zoom Out
            </Button>
            <span className="flex items-center px-2">{Math.round(currentScale * 100)}%</span>
            <Button variant="outline" size="sm" onClick={handleZoomIn} disabled={!onZoom || currentScale >= 3}>
        ...
        <div className="mt-auto">
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
            ...
            <ol className="text-sm text-yellow-700 list-decimal pl-4 space-y-1">
              ...
              <li>Click directly on the PDF to add the annotation</li>
              <li>Switch to Selection Mode (pointer tool) to select, drag, and delete annotations</li>
            </ol>
          </div>
          <Button
            variant="outline"
